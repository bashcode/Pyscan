...
def append_dirs_from_file(option, opt_str, value, parser):
    args = [arg.strip() for arg in open(value)]
    parser.values.include_dir.extend(args)

def append_users_from_file(option, opt_str, value, parser):
    args = [arg.strip() for arg in open(value)]
    parser.values.include_user.extend(args)
...
def parse_args():
    ...
    parser.add_option('--include-from-file', action='callback', type='string', callback=append_dirs_from_file, metavar='FILE',
            help='Include list of directories for scanning from FILE')
    parser.add_option('--include-user-from-file', action='callback', type='string', callback=append_users_from_file, metavar="FILE",
            help='Include list of users to scan public_html from FILE')
    ...
    if 'multiprocessing' in sys.modules:
	    ...
	    user_path = os.path.expanduser('~' + user) + '/public_html/')
            if os.path.exists(user_path):
                ...
                unsearched.put(os.path.expanduser(user_path)
            else:
